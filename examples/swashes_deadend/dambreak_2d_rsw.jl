...
alpha = (exp(-(damw/2)^2)-exp(-2*damw))/damh
beta = exp(-(damw/2)^2)/alpha-damh

phi(x,y) = x - ((y-l/2)^2)/(l^2) - damd
...
function get_ics(mesh)
	...
	for i in 1:mesh.nx, j in 1:mesh.ny
		...
		state.b[i,j] = min(0.5, max(0, exp(-phi(x,y)^2)/alpha-beta)) * mesh.A[i,j]
		state.h[i,j] = phi(x,y) <= 0 ? 0.5 * mesh.A[i,j] - state.b[i,j] : 0
	end
...
function test_conv(N)
	...
	#state = get_ics(mesh)
	state = State(mesh)
	state.h[mesh.nh+1:mesh.nx-mesh.nh, mesh.nh+1:mesh.ny-mesh.nh] .= depth * mesh.A[20,20]
	state.b[mesh.nh+1:mesh.nx-mesh.nh, mesh.nh+1:mesh.ny-mesh.nh] .= topo * mesh.A[20,20]

	#Creating the Model
	model = Model(rsw_rhs!, mesh, state, ["u_x", "u_y", "h"]; integratorstep! = rk3step!, cfl = 0.015, dtmax=0.015, Umax = get_Umax)
