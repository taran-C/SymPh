...
#Generating the RHS
rhs! = to_kernel(dtomega, dtb; explparams = explparams, verbose = false, bcs=[U, psi, dtomega, dtb])
...
for i in 1:ni, j in 1:nj
	...
	x0 = 0.5 * Lx
	#state.b[i,j] = y #* mesh.msk0d[i,j]
	state.b[i,j] += 0.01 * tanh(30 * (x-x0)) * mesh.msk0d[i,j]
end
...
run!(model; save_every = 10, tend = 250, maxite = 50000, writevars = (:u_i, :u_j, :omega, :b))
