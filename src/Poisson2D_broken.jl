...
function get_laplacian(mesh; o=2)
	if mesh.iperio & mesh.jperio
		return (get_codiff_i(mesh; factor=mesh.dy ./mesh.dx) * get_fvtofd_i(mesh)) * (get_diff_i(mesh) * get_fvtofd_i(mesh)) .* (mesh.dy ./mesh.dx)[10,10] + (get_codiff_j(mesh; factor=mesh.dx ./mesh.dy) * get_fvtofd_j(mesh)) * (get_diff_j(mesh) * get_fvtofd_j(mesh)) .* (mesh.dx./mesh.dy)[10,10] #WORKS FOR PERIODIC
	elseif o==4 
