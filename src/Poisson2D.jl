...
Poisson2D(bc, form_type; order=0) = Poisson2D(bc, form_type, nothing)
	...
	for j in 1:mesh.nj
		for i in 1:mesh.ni
			...
			if I>-1
				...
				west = i>mesh.nh ? G[i-1, j] : (mesh.iperio ? G[mesh.ni-mesh.nh, j] : -1)
				east = i<mesh.ni+1-mesh.nh ? G[i+1, j] : (mesh.iperio ? G[mesh.nh+1, j] : -1)
				south = j>mesh.nh ? G[i, j-1] : (mesh.jperio ? G[i, mesh.nj-mesh.nh] : -1)
				north = j<mesh.nj+1-mesh.nh ? G[i, j+1] : (mesh.jperio ? G[i, mesh.nh+1] : -1)
				...
				if location=="vertex"#bc == "dirichlet" #Closed border
					if location == "vertex"
				...
				elseif location=="center"#bc == "neumann" #Open border
